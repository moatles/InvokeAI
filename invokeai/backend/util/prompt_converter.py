...
class ScheduledPromptConditioning:
    """Represents a conditioning that's active until a specific step."""
    def __init__(self, end_at_step: int, cond):
...
def convert_a1111_emphasis_to_compel(text: str) -> str:
    ...
    A1111: [text] = 1/1.1x ≈ 0.909x (when not scheduling syntax)
    ...
    while i < len(text):
        ...
        if text[i] == '(':
            ...
            if depth == 0:
                ...
                # No weight - A1111 treats () as 1.1x multiplier
        ...
        if text[i] == '[':
            ...
            if depth == 0:
                ...
                # Check if it's scheduling syntax
                if colon_positions:
                ...
                # If scheduling or alternating, keep original
                if has_colon_number or has_pipe:
                    result.append(text[start:i])
                    continue

                # Plain [text] - de-emphasis
                converted_inner = convert_a1111_emphasis_to_compel(inner)
                result.append(f"({converted_inner})-")
                continue
...
def convert_prompt_for_compel(prompt: str) -> str:
    ...
    """
    return convert_a1111_emphasis_to_compel(prompt)
...
def get_cond_for_step(
    conditionings: List[Tuple[int, any]],
    step: int,
) -> any:
    """
    ...
    Args:
        conditionings: List of (end_at_step, conditioning) tuples
        step: Current step (1-indexed)

    Returns:
        The conditioning to use for this step
    """
    for end_step, cond in conditionings:
        if step <= end_step:
            return cond
    return conditionings[-1][1]


def reconstruct_cond_batch(
    cond_list: List[List[Tuple[int, any]]],
    current_step: int,
    device=None,
    dtype=None,
) -> 'torch.Tensor':
    ...
    conds = []
    for scheduled_conds in cond_list:
        cond = get_cond_for_step(scheduled_conds, current_step)
        conds.append(cond)

    # Stack, handling different lengths
    ...
    for c in conds:
        if c.shape[1] < max_len:
            padding = torch.zeros(
                ...
                device=c.device if device is None else device,
